...
int registerUser(Message *msg , int sockfd)
{
	...
	if(ret != SQLITE_OK)
	{
		printf("unable open database = %d.\n",ret);
		return FAILED;
	}//if
	printf("Opened database successfully.\n");
	
	char *szErrMsg = 0;
    char *szSql="create table User(userName text, password text, userAddr text, sockfd int, speak int, registerTime text);";
    ret=sqlite3_exec(db,szSql,0,0,&szErrMsg);	
	if(ret != SQLITE_OK)
	{
		printf("%s error!\n","create table");
        printf("%s\n",szErrMsg);  
		sqlite3_free(szErrMsg);
		if(!memcmp("table User already exists",szErrMsg,strlen(szErrMsg)))
		{
	...
	if(ret != SQLITE_OK)
	{
		...
		sqlite3_finalize(stmt);
		sqlite3_close(db);
		printf("FUNC = %s, line = %d ret = %d %d\n",__FUNCTION__ , __LINE__,ret,FAILED);
		return FAILED;
	}//if
	printf("FUNC = %s, line = %d \n",__FUNCTION__ , __LINE__);
	/*顺利注册*/
