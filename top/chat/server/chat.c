...
int groupChat(Message *msg , int sockfd)
{
	...
	else
	{
		...
		char *szErrMsg = 0;
		char *szSql="create table Message(msgType int, sendName text, recvName text, content text, msgTime text);";
		...
		if(ret != SQLITE_OK)
		{
			...
			if(!memcmp("table Message already exists",szErrMsg,strlen(szErrMsg)))
			{
